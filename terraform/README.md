...
Select workspace (development, staging, or production):

```bash
terraform workspace select development  # or create with: terraform workspace new development
```
...
- `production`: Production environment (higher availability, deletion protection)
- `staging`: Staging environment (pre-production testing)
- `development`: Development environment (reduced resources, easier cleanup)
...
# Switch environments
terraform workspace select production  # or staging, development
